...
export default class MediaModel extends BaseModel
{
    ...
    public insert(params: any, cb: Function): void {
        let query = 'INSERT INTO media (event_id, title, description, uploaded_by, status, filename, size, extension, playtime_string, playtime_seconds, asset_id, created_at, updated_at)'
    ...
    public deleteById(id: string, cb: Function): void {
        let query = 'UPDATE media SET status = :status, deleted_at = NOW(), updated_at = NOW() WHERE id = :id';
        let queryParams = {
            id: id,
            status: Constants.MEDIA_STATUS_DELETED
        };
